...
(async () => {
  ...
  try {
    ...
    const [existing] = await db.query('SELECT COUNT(*) as cnt FROM fee_types');
    if (existing[0].cnt === 0) {
      await db.query(`INSERT INTO fee_types (name_kh, name_en, amount, description, is_active) VALUES
        ...
        ('ថ្លៃប័ណ្ណសម្គាល់សិស្ស ឆ្នាំទី ១', 'Student ID Card Fee (Year 1)', 5000, 'ថ្លៃប័ណ្ណសម្គាល់សិស្ស (ឆ្នាំទី ១)', 1),
        ('ថ្លៃប័ណ្ណសម្គាល់សិស្ស ឆ្នាំទី ២', 'Student ID Card Fee (Year 2)', 5000, 'ថ្លៃប័ណ្ណសម្គាល់សិស្ស (ឆ្នាំទី ២)', 1),
        ('ថ្លៃសិក្សា', 'Tuition Fee', 500000, 'ថ្លៃសិក្សាមួយឆ្នាំសម្រាប់និស្សិតអាហារូបករណ៍', 1)
      `);
      console.log('Fee types seeded');
    }

    const [ft] = await db.query('SELECT * FROM fee_types');
    console.log('Total fee types: ' + ft.length);
    ft.forEach(f => console.log('  #' + f.id + ' | ' + f.name_kh + ' | ' + f.amount + '៛'));
